@extends('layouts.app')

@section('content')
<form action="{{route('update-page-section', $page_id)}}" method="post" class="form-submit">
@csrf
<input type="hidden" name="section_key" value="{{$section_key}}">
<input type="hidden" name="key" value="{{$id}}">

    <div class="d-flex justify-content-center mb-5">
        <div class="col-lg-9 mt-lg-0 mt-4">
    
            <div class="col-lg-12 mt-lg-0 mt-4">
                <div class="card mt-4">
                    <div class="card-header">
                        <h5>Our Team</h5>
                    </div>
                    <div class="card-body pt-0">
                        <div class="row mt-5">
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">Title</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="title" value="{{$data['title'] ?? ''}}">
                                    </div>
                                </div>
                            </div>
                           
                            <div class="col-12">
                                <hr>
                                <div class="form-group">
                                    <label class="form-label"><strong>Team List</strong></label>
                                    <div class="team_list" id="sortable_product">
                                        @if(isset($data['team_count']))
                                            @for($index = 0; $index < $data['team_count']; $index++)
                                                <div class="d-flex draggable"  id="row{{$index}}"  draggable="true" productID="{{$index}}">
                                                    <div class="col-lg-9 mt-lg-0">
                                                        <div class="card-body">
                                                            <select name="team_{{$index}}" id="team_{{$index}}" class="form-control team" data-index="{{$index}}">
                                                                <option value="">Select Team Member</option>    
                                                            {!! teamOptions($data['team_'.$index]) !!}
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div><button class="btn btn-sm btn-danger mt-4 team_member_remove"><i class="fa fa-trash"></i></button></div>
                                                </div>
                                            @endfor
                                        @else

                                            <div class="d-flex draggable"  id="row0"  draggable="true" productID="0">
                                                <div class="col-lg-9 mt-lg-0">
                                                    <div class="card-body">
                                                        <select name="team_0" id="team_0" class="form-control team" data-index="0">
                                                            <option value="">Select Team Member</option>    
                                                        {!! teamOptions() !!}
                                                        </select>
                                                    </div>
                                                </div>
                                                <div><button class="btn btn-sm btn-danger mt-4 team_member_remove"><i class="fa fa-trash"></i></button></div>
                                            </div>
                                        @endif
                                    </div>
                                    <button class="btn btn-sm btn-secondary team_member_add_more" data-tag="team" type="button">Add More</button>
                                    <input type="hidden" id="team_count" name="team_count" value="{{$data['team_count'] ?? '1'}}">
                                    <input type="hidden" id="team_list" name="team_list"  value="{{$data['team_list'] ?? ''}}">

                                </div>
                            </div>
                        </div>
    
                    </div>
                </div>
            </div>
            <div class="mt-5 float-right">
                            <button class="btn btn-primary">Submit</button>
                        </div>
        </div>
    </div>
</form>
@endsection
@push('js')
<script>
    $(document).on('click', '.team_member_add_more', function(e){
        e.preventDefault();
        if($('.draggable').length < 3) {

            $('.team_list').append(`<div class="d-flex draggable" >
                                                        <div class="col-lg-9 mt-lg-0 ">
                                                            <div class="card-body">
                                                                <select name="team_0" id="team_0" class="form-control team">
                                                                    <option value="">Select Team Member</option> 
                                                                    <?php echo teamOptions();?>
                                                                </select>
                                                            </div>
                                                        </div>
                                                        <div><button class="btn btn-sm btn-danger mt-4 team_member_remove"><i class="fa fa-trash"></i></button></div>
                                                    </div>`);
        }
      
                                            updateServiceSequance();
    });
    function updateServiceSequance(){
        let tag = 'team';
      
        $('.draggable').each(function(index, value){
            $(this).attr('id', 'row'+index).attr('productID', index);
            $(this).find('.'+tag).attr('name', tag+"_"+index).attr('id', tag+"_"+index).attr('data-index', index);
            
        });
        $('#team_count').val($('.'+tag).length);
        var values = [];
        $('.team').each(function(index, value){
            values.push($(this).val());
        }) 
        $('#team_list').val(JSON.stringify(values));
    }
    $(document).on('click', '.team_member_remove', function(e){
        $(this).parents().eq(1).remove();
        updateServiceSequance();
    });
    $(document).on('change', '.team', function(e){
       
        var services = $('#team_list').val();
        if(services != '') {
            services = JSON.parse(services);
        }
        else {
            services = [];
        }

        if($(this).val() != '') {
            
            if(services.indexOf($(this).val()) != -1) {
                $(this).val('').change();
                return false;
            }
           
        }
        updateServiceSequance();
       

    })
    </script>
@endpush